...
2) Get Prisma Client Go
    ...
    ```shell script
    go get github.com/fragdance1/prisma-client-go
    ```
...
3) Prepare your database schema in a `schema.prisma` file. For example, a simple schema with a sqlite database and
    ...
    generator db {
        provider = "go run github.com/fragdance1/prisma-client-go"
    ...
    go run github.com/fragdance1/prisma-client-go migrate dev --name init
    ```

    After the migration, the Prisma Client Go client is automatically generated in your project.

    If you just want to re-generate the client, run `go run github.com/fragdance1/prisma-client-go generate`.
