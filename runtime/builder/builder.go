...
func (q Query) ExecJson(ctx context.Context) (j []byte, err error) {
	payload := engine.GQLRequest{
		Query:     q.Build(),
		Variables: map[string]interface{}{},
	}
	fmt.Println(payload.Query)
	return q.DoJson(ctx, payload)
}

func (q Query) DoJson(ctx context.Context, payload interface{}) (j []byte, err error) {
	if q.Engine == nil {
		return nil, fmt.Errorf("client.Prisma.Connect() needs to be called before sending queries")
	}

	logger.Debug.Printf("[timing] building %q", time.Since(q.Start))

	err, j = q.Engine.DoJson(ctx, payload)
	now := time.Now()
